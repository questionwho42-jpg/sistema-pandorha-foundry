# Pandorha: Bestiário Narrativo Vol. 1B - O Sobrenatural (Níveis 1-5)

> _"A morte não é o fim em Pandorha. É apenas uma mudança de gerenciamento."_

Este tomo apresenta as criaturas sobrenaturais com estatísticas completas para combate tático e descrições detalhadas.

---

## 11. Família ÓssEE (A Legião Eterna)

### A. Esqueleto Guerreiro (Nível 1)

**História:**
Eles não são cadáveres quaisquer; são soldados que morreram em batalha e cujas almas ficaram presas à disciplina militar. Eles não sentem medo, dor ou dúvida. Mantêm a formação de escudo perfeitamente alinhada mesmo enquanto seus corpos são quebrados, e executam ordens complexas deixadas por generais mortos há séculos.

**Descrição Visual:**
Ossos amarelados limpos de qualquer vestígio de carne, unidos por magia negra invisível. Vestem restos de armaduras enferrujadas e tabardos desbotados de reinos esquecidos. As órbitas vazias ardem com pontinhos de luz vermelha que seguem os vivos com ódio frio. Movem-se com tiques mecânicos, produzindo um som de "clac-clac" enervante.

**Ficha Técnica:**

- **Iniciativa:** -2 (Mental 0 + Inter -2) | **Sentidos:** Visão no Escuro (18m) | **Idioma:** -
- **HP:** 20 | **CA:** 13 (Armadura Quebrada) | **Movimento:** 9m.
- **Eixos:** Físico +2, Mental 0, Social -1.
- **Aplicações:** Conflito +2, Resistência +2, Interação -2.
- **Passiva - Ossos Secos:** Possui **RD 5** contra Dano Perfurante (flechas passam entre as costelas). Sofre **+1d6 de Dano** de armas de Esmagamento (Vulnerável).
- **[A] Espada Antiga:** +3 (Nv 1 + Fis 2). Alcance 1,5m. Dano **1d6+2 Perfurante**.

### B. Esqueleto Arqueiro (Nível 1)

**História:**
Em vida, eram batedores e caçadores de elite. Na morte, tornaram-se sentinelas incansáveis que vigiam as muralhas de fortalezas negras. Eles não enxergam com olhos, mas sentem a pulsação da vida à distância, disparando flechas de osso com precisão sobrenatural.

**Descrição Visual:**
Mais leves e ágeis que os guerreiros, com ossos porosos manchados de terra e musgo. Carregam arcos feitos de madeira podre ou costelas curvas. Preferem locais elevados, ficando imóveis como gárgulas até o momento do disparo.

**Ficha Técnica:**

- **Iniciativa:** -2 (Mental 0 + Inter -2) | **Sentidos:** Visão no Escuro (18m) | **Idioma:** -
- **HP:** 18 | **CA:** 12 | **Movimento:** 9m.
- **Eixos:** Físico +2, Mental 0, Social -1.
- **Aplicações:** Conflito +1, Resistência +1, Interação -2.
- **[A] Arco Curto:** +3 (Nv 1 + Fis 2). Alcance 24m. Dano **1d6+2 Perfurante**.

### C. Guarda de Elite (Nível 3)

**História:**
Os restos mortais de paladinos caídos ou guardas reais juramentados. Sua lealdade era tão forte que nem a morte a quebrou. Guardam tumbas de reis antigos e tesouros malditos, recusando-se a deixar seus postos mesmo quando o teto desaba sobre eles.

**Descrição Visual:**
Imponente e assustador, com quase 2 metros de altura. Usa uma armadura de placas completa, polida magicamente para brilhar nas trevas, e empunha um escudo torre pesado. Sua voz, quando fala, é metálica e oca, ecoando como se viesse de dentro de um barril de ferro.

**Ficha Técnica:**

- **Iniciativa:** +2 (Mental +1 + Inter +1) | **Sentidos:** Visão no Escuro (18m) | **Idioma:** Comum (Antigo).
- **HP:** 40 | **CA:** 18 (Placas + Escudo) | **Movimento:** 9m.
- **Eixos:** Físico +4, Mental +1, Social 0.
- **Aplicações:** Conflito +4, Resistência +3, Interação +1.
- **Passiva - Parede de Ossos:** Fornece **Cobertura Total** (+5 CA) para aliados de tamanho igual ou menor que estejam diretamente atrás dele.
- **[A] Espada Longa:** +7 (Nv 3 + Fis 4). Dano **1d8+4 Cortante**.

---

## 12. Família Pútrida (A Fome)

### A. Zumbi Pútrido (Nível 1)

**História:**
O degrau mais baixo da necromancia. Corpos recém-enterrados reanimados sem alma, movidos apenas por uma fome mágica insaciável. Eles são lentos e estúpidos, mas implacáveis; se você cortar um ao meio, a parte de cima continuará rastejando para morder seu tornozelo.

**Descrição Visual:**
Pele cinza-esverdeada rasgada em vários pontos, com vermes brancos expostos na carne. Um gemido molhado e constante escapa de suas gargantas. O cheiro é adocicado e insuportável, uma mistura de flores de funeral e carne estragada.

**Ficha Técnica:**

- **Iniciativa:** -4 (Mental -2 + Inter -2) | **Sentidos:** Visão no Escuro (9m) | **Idioma:** -
- **HP:** 25 | **CA:** 9 (Lento) | **Movimento:** 6m.
- **Eixos:** Físico +3, Mental -2, Social -2.
- **Aplicações:** Conflito +2, Resistência +4, Interação -2.
- **Passiva - Resistência Morta:** Se chegar a 0 HP, faz um **Teste Global de Físico + Resistência + Nível (DC 5 + Dano sofrido)**. Se passar, volta com **1 HP**. (Dano Radiante e Crítico anulam isso).
- **Passiva:** **Vulnerável a Radiante (+1d6 Dano)**.
- **[A] Pancada:** +4 (Nv 1 + Fis 3). Dano **1d6+3 Esmagamento**.

### B. Zumbi Inchado (Nível 2)

**História:**
Crias de pântanos ou esgotos, cheios de gases da decomposição e alquimia vil. Necromantes os usam como "bombas biológicas" para quebrar linhas de defesa inimigas, pois matá-los é tão perigoso quanto deixá-los viver.

**Descrição Visual:**
Grotescamente obeso, com a pele translúcida e esticada ao limite. Um brilho verde doentio pulsa em sua barriga. Líquido necrótico vaza constantemente de seus orifícios.

**Ficha Técnica:**

- **Iniciativa:** -4 (Mental -2 + Inter -2) | **Sentidos:** Visão no Escuro (9m) | **Idioma:** -
- **HP:** 30 | **CA:** 8 | **Movimento:** 6m.
- **Eixos:** Físico +2, Mental -2, Social -2.
- **Aplicações:** Conflito +2, Resistência +3, Interação -2.
- **Passiva - Explosão de Gás:** Quando morre, explode num raio de 3m. Todos na área fazem um **Teste Global de Físico + Resistência + Nível (DC 14)** (10 + Nv 2 + 2 Fis) ou sofrem **2d6 Veneno**.
- **[A] Vômito Ácido:** +4 (Nv 2 + Fis 2). Alcance 3m. Dano **2d4 Ácido**.

### C. Corredor (Nível 3)

**História:**
Cadáveres muito frescos reanimados com rituais de adrenalina e ódio concentrado. Eles não sofrem o rigor mortis; pelo contrário, seus músculos estão em espasmo perpétuo. Caçam em bandos como lobos mortos-vivos.

**Descrição Visual:**
Olhos injetados de sangue negro, veias saltadas por todo o corpo. Corre de quatro apoios como um animal, com movimentos bruscos e erráticos que dificultam a mira.

**Ficha Técnica:**

- **Iniciativa:** -3 (Mental -1 + Inter -2) | **Sentidos:** Visão no Escuro (18m) | **Idioma:** -
- **HP:** 35 | **CA:** 14 | **Movimento:** 15m.
- **Eixos:** Físico +4, Mental -1, Social -2.
- **Aplicações:** Conflito +3, Resistência +4, Interação -2.
- **Passiva - Impulso Necrótico:** No primeiro turno de combate, o Corredor ganha **1 Ação [A] extra** que deve ser usada exclusivamente para Mover (Totalizando 4 Ações no turno).
- **[AA] Mordidas:** +7 (Nv 3 + Fis 4). Dois ataques. Dano **1d6+4 Perfurante** cada.

---

## 13. Família AracnídEE (Filhas da Sombra)

### A. Aranha Gigante (Nível 2)

**História:**
Predadores perfeitos crescidos por magia selvagem nas profundezas das florestas antigas. Elas não distinguem um cervo de um elfo; ambos são apenas pacotes de comida quente para serem estocados em suas teias pegajosas.

**Descrição Visual:**
Quitina negra e brilhante coberta de pelos sensoriais rígidos. Possui 8 olhos sem alma que refletem a luz e mandíbulas que gotejam veneno verde. Move-se pelas paredes com um som de arranhar seco.

**Ficha Técnica:**

- **Iniciativa:** +1 (Mental +1 + Inter 0) | **Sentidos:** Visão no Escuro, Vibração (18m) | **Idioma:** -
- **HP:** 28 | **CA:** 14 (Natural) | **Movimento:** 12m / 12m Escalada.
- **Eixos:** Físico +3, Mental +1, Social 0.
- **Aplicações:** Conflito +2, Resistência +2, Interação 0.
- **Passiva:** Anda em Paredes e Teias sem penalidade de terreno difícil.
- **[A] Picada:** +5 (Nv 2 + Fis 3). Dano **1d8+3 Perfurante** + **2d8 Veneno**. (**Teste Global de Físico + Resistência + Nível (DC 15)** (10 + Nv 2 + 3 Fis) reduz o **dano de** veneno à metade).

### B. Tecelã de Fase (Nível 4)

**História:**
Aranhas raras que aprenderam a caçar entre os planos Material e Etéreo. Elas surgem do nada para dar o golpe fatal e desaparecem antes que a espada do inimigo possa tocá-las. Magos pagam fortunas por sua seda.

**Descrição Visual:**
Cores iridescentes mudam conforme o ângulo de visão. Seu corpo parece tremeluzir como uma imagem falha ou uma miragem.

**Ficha Técnica:**

- **Iniciativa:** +2 (Mental +2 + Inter 0) | **Sentidos:** Visão no Escuro (18m) | **Idioma:** -
- **HP:** 35 | **CA:** 15 | **Movimento:** 12m / 12m Escalada.
- **Eixos:** Físico +3, Mental +2, Social 0.
- **Aplicações:** Conflito +3, Resistência +2, Interação 0.
- **Passiva - Salto Etéreo:** Pode usar 1 Ação [L] para ficar invisível e intangível até o início do seu próximo turno.
- **[A] Picada Dimensional:** +7 (Nv 4 + Fis 3). Dano **2d10+3 Força**. O alvo **não soma bônus de Escudo ou Armadura** na sua CA contra este ataque (Toque).

### C. Rainha do Ninho (Nível 5)

**História:**
Uma aranha antiga que viveu séculos consumindo aventureiros e monstros. Comanda o enxame como uma general, usando telepatia primitiva. Seu covil é um labirinto tridimensional de seda branca.

**Descrição Visual:**
Enorme (Tamanho Grande), com o abdômen inchado e pulsante cheio de ovos. Suas patas são blindadas com quitina grossa como aço.

**Ficha Técnica:**

- **Iniciativa:** +3 (Mental +2 + Inter +1) | **Sentidos:** Visão no Escuro, Telepatia | **Idioma:** -
- **HP:** 70 | **CA:** 16 | **Movimento:** 12m Escalada.
- **Eixos:** Físico +5, Mental +2, Social +1.
- **Aplicações:** Conflito +4, Resistência +3, Interação +1.
- **[AA] Disparo de Teia:** +10 (Nv 5 + Fis 5). Alcance 18m. Dano **2d6+5 Impacto**. O alvo fica **Imobilizado** (**Teste Global de Físico + Resistência + Nível (DC 20)** (10 + Nv 5 + 5 Fis) para soltar).

---

## 14. Família Carniceira (Ghouls)

### A. Carniçal Faminto (Nível 3)

**História:**
Humanos que cometeram o tabu do canibalismo e foram amaldiçoados por deuses da morte. Mantêm uma inteligência astuta e memórias de sua vida, mas são escravos biológicos de uma fome que nunca passa.

**Descrição Visual:**
Emaciado, com membros alongados e dedos que terminam em garras de osso. A língua é preta e comprida, lambendo dentes que foram lixados até virarem agulhas.

**Ficha Técnica:**

- **Iniciativa:** +1 (Mental +1 + Inter 0) | **Sentidos:** Visão no Escuro (18m) | **Idioma:** Comum.
- **HP:** 30 | **CA:** 12 | **Movimento:** 9m.
- **Eixos:** Físico +3, Mental +1, Social 0.
- **Aplicações:** Conflito +3, Resistência +2, Interação 0.
- **[A] Garra Paralisante:** +6 (Nv 3 + Fis 3). Dano **2d4+3**. Se o alvo for uma criatura viva (não elfo), deve fazer um **Teste Global de Físico + Resistência + Nível (DC 16)** (10 + Nv 3 + 3 Fis).
  - **Sucesso:** Alvo fica **Abalado** (Penalidade -2) por 1 rodada.
  - **Falha:** Alvo fica **Imobilizado/Paralisado** por **1 rodada** (até o fim do próximo turno do alvo).
  - **Falha Crítica:** Alvo fica **Imobilizado** por **1d4+1 rodadas**.

### B. Carniçal Rastejante (Nível 4)

**História:**
Uma variante adaptada para viver em criptas apertadas e tubulações. Seus membros foram quebrados e curados de forma errada para permitir movimentos impossíveis. Eles caem do teto sobre suas vítimas.

**Descrição Visual:**
Seus joelhos e cotovelos dobram para ambos os lados. Tem ventosas de carne nas palmas das mãos e pés.

**Ficha Técnica:**

- **Iniciativa:** +1 (Mental +1 + Inter 0) | **Sentidos:** Visão no Escuro (18m) | **Idioma:** Comum.
- **HP:** 45 | **CA:** 14 | **Movimento:** 12m Escalada.
- **Eixos:** Físico +4, Mental +1, Social 0.
- **Aplicações:** Conflito +3, Resistência +3, Interação 0.
- **[A] Bote:** Pula até 6m sobre um alvo. Se acertar o ataque, o alvo cai **Caído** e o Carniçal tem uma ação de Ataque extra gratuita.

### C. Ghast (Nível 5)

**História:**
Um lorde entre os carniceiros. Quando um ghoul consome carne suficiente de heróis ou nobres, ele evolui. Exala um cheiro de morte tão potente que faz os vivos vomitarem e ficarem indefesos.

**Descrição Visual:**
Maior e mais musculoso que seus primos, com a pele úmida e pálida que emite um vapor visível de podridão.

**Ficha Técnica:**

- **Iniciativa:** +3 (Mental +2 + Inter +1) | **Sentidos:** Visão no Escuro (18m) | **Idioma:** Comum.
- **HP:** 60 | **CA:** 14 | **Movimento:** 9m.
- **Eixos:** Físico +4, Mental +2, Social +1.
- **Aplicações:** Conflito +4, Resistência +4, Interação +1.
- **Passiva - Fedor:** Aura 3m. Criaturas vivas que começam o turno na área fazem **Teste Global de Físico + Resistência + Nível (DC 18)** (10 + Nv 5 + 3 Fis). Falha: sofrem **-2 de Penalidade** em todos os testes e ataques (Envenenado).
- **[AA] Frenesi:** +9. Dois ataques de garra **2d6+4** cada.

---

## 15. Família Amorfa (Limos)

### A. Geleia Ocre (Nível 3)

**História:**
Uma colônia ácida com mente coletiva primitiva. Elas limpam masmorras de toda matéria orgânica, deixando apenas pedra e metal limpos. Não podem ser mortas por cortes simples.

**Descrição Visual:**
Uma poça de lodo amarelo-mostarda que escorre pelas paredes e pelo teto, desafiando a gravidade.

**Ficha Técnica:**

- **Iniciativa:** -8 (Mental -4 + Inter -4) | **Sentidos:** Visão Cega (9m) | **Idioma:** -
- **HP:** 45 | **CA:** 8 | **Movimento:** 6m / 6m Escalada.
- **Eixos:** Físico +2, Mental -4, Social -4.
- **Aplicações:** Conflito +2, Resistência +4, Interação -4.
- **Passiva:** **Vulnerável a Elétrico (+1d6)**. **Imune a Corte** (Se cortada, divide em duas gelatinas menores com metade do HP cada).
- **[A] Pseudópodo:** +5. Dano **1d6+2 Ácido**.

### B. Cubo Gelatinoso (Nível 4)

**História:**
O predador perfeito de corredores de masmorra. Sua transparência é sua maior arma; aventureiros distraídos costumam andar diretamente para dentro dele, onde são digeridos vivos enquanto flutuam paralisados.

**Descrição Visual:**
Um cubo perfeito de 3x3 metros de gelatina transparente. Olhando de perto, pode-se ver ossos, moedas e armas de vítimas anteriores flutuando em seu interior.

**Ficha Técnica:**

- **Iniciativa:** -8 (Mental -4 + Inter -4) | **Sentidos:** Visão Cega (18m) | **Idioma:** -
- **HP:** 85 | **CA:** 6 | **Movimento:** 4,5m.
- **Eixos:** Físico +4, Mental -4, Social -4.
- **Aplicações:** Conflito +3, Resistência +5, Interação -4.
- **Passiva - Transparente:** Exige **Teste Global de Mental + Interação + Nível (Sentidos) DC 15** para ser notado se estiver imóvel.
- **[A] Engolfar:** Avança sobre criaturas. **Teste Global de Físico + Resistência + Nível (DC 18)** (10 + Nv 4 + 4 Fis) ou é absorvido. Vítima sofre **3d6 Ácido por turno e fica Agarrada (e sem ar)**.

### C. Pudim Negro (Nível 5)

**História:**
O limo supremo. Ao contrário dos outros, possui uma inteligência maligna e odeia a vida. Seu ácido é tão potente que dissolve metal e magia em segundos.

**Descrição Visual:**
Uma massa de piche negro borbulhante que parece absorver a luz ao seu redor.

**Ficha Técnica:**

- **Iniciativa:** -6 (Mental -2 + Inter -4) | **Sentidos:** Visão Cega (18m) | **Idioma:** -
- **HP:** 90 | **CA:** 7 | **Movimento:** 6m.
- **Eixos:** Físico +5, Mental -2, Social -4.
- **Aplicações:** Conflito +4, Resistência +5, Interação -4.
- **Passiva - Corrosão:** Armas não-mágicas de metal toque tocam nele derretem (recebem penalidade de -1 Dano permanente, se chegar a -5 quebra).
- **[A] Pancada Ácida:** +10 (Nv 5 + Fis 5). Dano **2d6+5** + **2d8 Ácido**.

---

## 16. Família Sombria

### A. Sombra (Nível 2)

**História:**
A sombra de alguém que morreu de medo extremo, ganhando vida própria. Elas se alimentam da força vital dos vivos, transformando campeões em fracos trêmulos.

**Descrição Visual:**
Uma silhueta bidimensional negra que desliza pelas superfícies, mas que pode se erguer no ar como um rasgo na realidade.

**Ficha Técnica:**

- **Iniciativa:** 0 (Mental +1 + Inter -1) | **Sentidos:** Visão no Escuro (18m) | **Idioma:** -
- **HP:** 16 | **CA:** 12 | **Movimento:** 12m.
- **Eixos:** Físico +2 (Dreno), Mental +1, Social -1.
- **Aplicações:** Conflito +2, Resistência +2, Interação -1.
- **Passiva:** **RD 5 (Físico não-mágico)**. **Vulnerável a Radiante (+1d6)**.
- **[A] Toque:** +4. Dano **2d6 Necrótico**. A vítima perde **1d4 de Força** temporariamente (recupera no descanso). Se Força chegar a 0, morre.

### B. Espectro (Nível 4)

**História:**
A alma de alguém que morreu com ódio violento e inacabado. Odeiam a luz do sol e qualquer lembrança de vida. Sua mera presença suga a vontade de viver.

**Descrição Visual:**
Um fantasma translúcido cinza, flutuando sem pés. Seu rosto é uma máscara de fúria contorcida.

**Ficha Técnica:**

- **Iniciativa:** +3 (Mental +2 + Inter +1) | **Sentidos:** Visão no Escuro (30m) | **Idioma:** Comum.
- **HP:** 45 | **CA:** 13 | **Movimento:** 15m Voo (Hover).
- **Eixos:** Físico +3, Mental +2, Social 0.
- **Aplicações:** Conflito +4, Resistência +3, Interação +1.
- **Passiva:** Etéreo (Atravessa paredes).
- **[A] Dreno de Vida:** +7. Dano **3d6 Necrótico**. O alvo tem seu **HP Máximo reduzido** pelo mesmo valor do dano.

### C. Banshee (Nível 5)

**História:**
O espírito de uma mulher nobre corrompida pela vaidade e egoísmo. Ela foi amaldiçoada a lamentar eternamente sua própria beleza perdida. Ouvir seu grito é a morte certa.

**Descrição Visual:**
Uma figura feminina flutuante, bela mas terrível, com cabelos flutuando em água invisível e rosto distorcido em um grito eterno.

**Ficha Técnica:**

- **Iniciativa:** +8 (Mental +4 + Inter +4) | **Sentidos:** Visão no Escuro | **Idioma:** Élfico, Comum.
- **HP:** 55 | **CA:** 12 | **Movimento:** 12m Voo.
- **Eixos:** Mental +4, Social +4.
- **Aplicações:** Conflito 0, Resistência +4, Interação +4.
- **[A] Lamento:** (1/dia). Todas as criaturas vivas a 9m devem passar em **Teste Global de Físico + Resistência + Nível (DC 19)** (10 + Nv 5 + 4 Men/Soc). 
  - **Falha:** Sofre **8d6 de Dano Psíquico** e fica **Abalado** (Penalidade -2 em todos os testes) por 1 minuto.
  - **Sucesso:** Sofre metade do dano e não fica Abalado.

---

## 17. Família Cultista

### A. Iniciado (Nível 1)

**História:**
Aldeões comuns seduzidos pela promessa de poder fácil ou proteção. Eles são descartáveis para seus mestres e sabem disso, lutando com o desespero de quem não tem volta.

**Descrição Visual:**
Vestem túnicas simples com capuz para esconder o rosto. Carregam adagas rituais tortas e têm símbolos profanos marcados na pele.

**Ficha Técnica:**

- **Iniciativa:** +2 (Mental +1 + Inter +1) | **Sentidos:** Visão Normal | **Idioma:** Comum.
- **HP:** 10 | **CA:** 11 | **Movimento:** 9m.
- **Eixos:** Físico +1, Mental +1, Social +1.
- **Aplicações:** Conflito +1, Resistência 0, Interação +1.
- **[A] Adaga:** +2. Dano **1d4+1**.

### B. Fanático (Nível 3)

**História:**
Membros do culto que passaram pelos ritos de iniciação sangrentos. Sua mente foi quebrada e reconstruída para lealdade absoluta. Eles lutam com uma fúria suicida, ignorando ferimentos mortais.

**Descrição Visual:**
Cobertos de cicatrizes rituais e escarificações. Seus olhos são vidrados de loucura. Murmuram orações constantes para entidades sombrias.

**Ficha Técnica:**

- **Iniciativa:** +5 (Mental +3 + Inter +2) | **Sentidos:** Visão Normal | **Idioma:** Comum, Infernal.
- **HP:** 33 | **CA:** 13 | **Movimento:** 9m.
- **Eixos:** Físico +2, Mental +3, Social +2.
- **Aplicações:** Conflito +2, Resistência +3, Interação +2.
- **[AA] Magia:** _Infligir Ferimentos_ (+6 toque, **3d10 Necrótico**).

### C. Mestre do Pacto (Nível 5)

**História:**
Um líder que fez contato direto com demônios ou deuses antigos. Ele vendeu sua alma por poder etérico real e comanda a célula do culto com mão de ferro e magia negra.

**Descrição Visual:**
Veste mantos de seda negra e carrega um cajado feito de ossos humanos. Uma aura de frio o cerca.

**Ficha Técnica:**

- **Iniciativa:** +9 (Mental +4 + Inter +5) | **Sentidos:** Visão no Escuro (Bênção) | **Idioma:** Comum, Infernal, Abissal.
- **HP:** 60 | **CA:** 15 (Armadura Mágica) | **Movimento:** 9m.
- **Eixos:** Físico +2, Mental +4, Social +4.
- **Aplicações:** Conflito +3, Resistência +3, Interação +5.
- **[AA] Rajada Mística:** +9 (Nv 5 + Men 4). Alcance 36m. Dano **1d10+4 Força**.
- **[AA] Magia Negra:** _Bola de Fogo_ (8d6) ou _Escuridão_.

---

## 18. Família Híbrida (Monstruosidades)

### A. Harpia (Nível 3)

**História:**
Seres amaldiçoados pela vaidade ou criados por deuses do vento furiosos. Elas vivem em penhascos costeiros e usam seu canto mágico para atrair viajantes para a morte nas pedras.

**Descrição Visual:**
Corpo de abutre sujo com o rosto de uma velha enrugada e perversa. Suas garras estão sempre incrustadas de sangue seco.

**Ficha Técnica:**

- **Iniciativa:** +4 (Mental +1 + Inter +3) | **Sentidos:** Visão Normal | **Idioma:** Comum, Auran.
- **HP:** 38 | **CA:** 11 | **Movimento:** 6m / 12m Voo.
- **Eixos:** Físico +3, Mental +1, Social +3.
- **Aplicações:** Conflito +3, Resistência +2, Interação +3.
- **Passiva - Canção:** Alvos a 30m devem fazer **Teste Global de Mental + Resistência + Nível (DC 16)** (10 + Nv 3 + 3 Soc) ou ficam **Encantados**. Enquanto encantados, devem gastar suas Ações de Movimento para se aproximar da Harpia pelo caminho mais seguro. (Dura 1 minuto ou até a Harpia ser atacada).
- **[AA] Garras:** +6. Dano **2d4+3**.

### B. Sirena (Nível 4)

**História:**
Primas aquáticas das harpias que caçam marinheiros. Elas não querem apenas carne; elas querem afogar a esperança. Constroem seus ninhos em naufrágios.

**Descrição Visual:**
Belas de longe, monstruosas de perto. Têm penas azuis oleosas e guelras no pescoço.

**Ficha Técnica:**

- **Iniciativa:** +6 (Mental +2 + Inter +4) | **Sentidos:** Visão no Escuro | **Idioma:** Comum, Aquan.
- **HP:** 50 | **CA:** 12 | **Movimento:** 9m / 18m Nado.
- **Eixos:** Físico +3, Mental +2, Social +4.
- **Aplicações:** Conflito +3, Resistência +2, Interação +4.
- **[AA] Grito Sônico:** Cone 4m. **3d6 Trovão**. Alvos que falharem no **Teste Global de Físico + Resistência + Nível (DC 14)** ficam Atordoados.

### C. Matriarca dos Ventos (Nível 5)

**História:**
A rainha do ninho, abençoada com o controle sobre o clima. Ela pode convocar tempestades localizadas para derrubar navios ou caravanas.

**Descrição Visual:**
Usa uma coroa feita de ossos de dedos e suas asas são negras como nuvens de tempestade.

**Ficha Técnica:**

- **Iniciativa:** +7 (Mental +3 + Inter +4) | **Sentidos:** Visão no Escuro | **Idioma:** Comum, Auran.
- **HP:** 70 | **CA:** 14 | **Movimento:** 15m Voo.
- **Eixos:** Físico +4, Mental +3, Social +3.
- **Aplicações:** Conflito +4, Resistência +3, Interação +4.
- **[AA] Raio:** +8 (Nv 5 + Men 3). Dano **3d10 Elétrico**.

---

## 19. Família Feérica

### A. Pixie (Nível 1)

**História:**
Espíritos da natureza minúsculos e travessos. Eles não entendem o conceito de propriedade ou mortalidade, pregando peças que podem ser fatais para humanos sem querer.

**Descrição Visual:**
Apenas 30cm de altura, com asas de inseto brilhantes (borboleta ou libélula). Deixam rastro de pó dourado por onde passam e sua pele brilha suavemente à noite.

**Ficha Técnica:**

- **Iniciativa:** +6 (Mental +3 + Inter +3) | **Sentidos:** Visão na Penumbra | **Idioma:** Silvestre.
- **HP:** 3 | **CA:** 15 (Tamanho) | **Movimento:** 9m Voo.
- **Eixos:** Físico -2, Mental +3, Social +3.
- **Aplicações:** Conflito 0, Resistência 0, Interação +3.
- **Passiva:** Pode ficar Invisível à vontade (1 Ação [L]).
- [AA] Pó Mágico: Alcance 3m. Alvo faz **Teste Global de Mental + Resistência + Nível (DC 14)** (10 + Nv 1 + 3 Men/Soc). 
  - **Sucesso:** Alvo fica **Abalado** (Penalidade -2) por 1 rodada.
  - **Falha:** Cai em **Sono** (Inconsciente por 1 minuto ou até sofrer dano).
  - **Falha Crítica:** Sono Profundo (Exige 1 Ação de aliado para acordar, além de dano).
- **[A] Espinho:** +6 (Nv 1 + Men 3 + 2). Alcance 6m. Dano **1 Perfurante** + **1d4 Veneno**.

### B. Sprite de Guerra (Nível 3)

**História:**
Os guardiões militantes das florestas feéricas. Eles levam a proteção da natureza a sério e atacam lenhadores ou invasores com venenos de sono.

**Descrição Visual:**
Vestem armaduras feitas de carapaças de besouro e carregam espadas finas como agulhas. Seus rostos são belos, mas duros e sem piedade.

**Ficha Técnica:**

- **Iniciativa:** +3 (Mental +2 + Inter +1) | **Sentidos:** Visão na Penumbra | **Idioma:** Silvestre.
- **HP:** 20 | **CA:** 15 | **Movimento:** 12m Voo.
- **Eixos:** Físico +4 (Des), Mental +2, Social +1.
- **Aplicações:** Conflito +4, Resistência +2, Interação +1.
- **[A] Espada:** +7 (Nv 3 + Fis 4). Dano 1 Perfurante + **3d6 Veneno** (Se falhar no **Teste Global de Físico + Resistência + Nível (DC 14)**, o alvo cai **Inconsciente** por 1 hora ou até sofrer dano).

### C. Dríade (Nível 5)

**História:**
Um espírito ligado à alma de uma árvore antiga (geralmente um carvalho). Se a árvore morre, ela morre. Ela pode encantar intrusos para servirem como adubo.

**Descrição Visual:**
Sua pele parece casca de árvore polida, e seus cabelos mudam de cor com as estações (verde, dourado, branco). Seus olhos são lagos profundos de verde antigo.

**Ficha Técnica:**

- **Iniciativa:** +9 (Mental +4 + Inter +5) | **Sentidos:** Visão no Escuro | **Idioma:** Silvestre, Élfico.
- **HP:** 45 | **CA:** 16 (Casca) | **Movimento:** 9m.
- **Eixos:** Físico +2, Mental +4, Social +4.
- **Aplicações:** Conflito +2, Resistência +4, Interação +5.
- **Passiva:** Teleporte entre árvores (1 Ação [L]).
- **[A] Charme:** **Teste Global de Mental + Interação + Nível (DC 19)** (10 + Nv 5 + 4 Soc) ou o alvo fica controlado por ela.
- **[A] Chicote de Vinha:** +8 (Nv 5 + Fis 2). Alcance 4,5m. Dano **1d8+2 Contundente**.

---

## 20. Família Artificial

### A. Espantalho (Nível 2)

**História:**
Bonecos de palha animados por espíritos malignos ou bruxas para vigiar campos. Eles não sentem dor e não param até que sejam queimados ou desmembrados. Seu olhar pode paralisar corvos e homens de medo.

**Descrição Visual:**
Um boneco tosco cheio de palha seca, vestindo roupas de fazendeiro podres. Sua cabeça é um saco de estopa costurado ou uma abóbora murcha esculpida com um sorriso cruel. Move-se de forma espasmódica e antinatural.

**Ficha Técnica:**

- **Iniciativa:** +2 (Mental 0 + Inter +2) | **Sentidos:** Visão no Escuro | **Idioma:** Compreende Comum (não fala).
- **HP:** 25 | **CA:** 11 | **Movimento:** 9m.
- **Eixos:** Físico +2, Mental 0, Social +2.
- **Aplicações:** Conflito +2, Resistência +2, Interação +2.
- **Passiva:** **Vulnerável a Fogo (+1d6)**. **RD 5 (Contundente)**.
- **Passiva - Olhar Aterrorizante:** Inimigos que começam o turno a 9m devem passar em **Teste Global de Mental + Resistência + Nível (DC 14)** (10 + Nv 2 + 2 Soc) ou ficam **Aterrorizados** por 1 minuto (Teste novo no fim de cada turno).
- **[A] Garra:** +4. Dano **1d8+2**.

### B. Armadura Animada (Nível 3)

**História:**
Uma armadura vazia movida por magia. Frequentemente guardam corredores de castelos e tesouros de magos, fingindo ser decoração até atacarem. Elas seguem ordens literalmente e lutam até serem destruídas.

**Descrição Visual:**
Uma armadura de placas completa antiga, mas sem ninguém dentro. Ouve-se o rangido de metal contra metal e o som oco quando ela se move. Elmos vazios que parecem olhar para você.

**Ficha Técnica:**

- **Iniciativa:** 0 (Mental 0 + Inter 0) | **Sentidos:** Visão Cega (18m) | **Idioma:** -
- **HP:** 33 | **CA:** 18 (Placas) | **Movimento:** 9m.
- **Eixos:** Físico +4, Mental 0, Social 0.
- **Aplicações:** Conflito +3, Resistência +3, Interação 0.
- **Passiva:** Imune a Psíquico, Veneno, Doença. **RD 5 (Corte)**.
- **[AA] Espada:** +7. Dano **1d8+4**.

### C. Gárgula de Bronze (Nível 5)

**História:**
Uma construção mais cara e letal que a gárgula de pedra, feita de metal puro. Usada por magos ricos e artífices paranóicos. Diferente da pedra, o metal é mais ágil e resistente.

**Descrição Visual:**
Uma estátua alada feita de bronze polido ou oxidado (verde-azulado). Engrenagens e pistões mágicos podem ser ouvidos zumbindo em seu interior. Seus olhos brilham como fornalhas vermelhas.

**Ficha Técnica:**

- **Iniciativa:** 0 (Mental 0 + Inter 0) | **Sentidos:** Visão no Escuro (18m) | **Idioma:** Terran (Dracônico?).
- **HP:** 60 | **CA:** 17 | **Movimento:** 12m Voo.
- **Eixos:** Físico +5, Mental 0, Social 0.
- **Aplicações:** Conflito +5, Resistência +4, Interação 0.
- **Passiva:** **RD 8 (Dano Físico não-mágico)**.
- **[AA] Garras:** +10 (Nv 5 + Fis 5). Dano **1d10+5 Corte**.

