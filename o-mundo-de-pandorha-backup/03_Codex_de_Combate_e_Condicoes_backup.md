...
## 🏗️ 1. O Fluxo de Batalha (3 Ações)

### 1.0 Iniciativa (Mental + Interação)

Antes do primeiro turno, todos rolam **Iniciativa** para decidir a ordem de agir.

- **Fórmula:** [d20] + [Eixo Mental] + [Aplicação Interação].
...
Use **[Físico + Conflito]** contra a **DC Passiva** do alvo (10 + Nível + Físico + Conflito).
...
| 20  | **Inconsciente** | Incapacitado, Caído e larga itens. Falha automática em testes de Resistência. Ataques adjacentes são Críticos.             | Acordar com dano ou se receber cura.                                                                                |
...
| 25  | **Moribundo**    | Inconsciente c/ 0 HP. Deve fazer Testes de Morte (Físico + Resistência) no final do turno vs DC (10 + Nível).              | 3 Sucessos estabilizam. 3 Falhas = Morte. Ajuda: [Mental + Interação] vs DC (10 + Nível do Moribundo).              |
...
| 30  | **Sangrando**    | 1d4 dano/turno. Não recupera HP naturalmente.                                                                              | Teste Global de [Físico + Resistência + Nível DC da Fonte] (Fim do Turno).                                          |
